...
let rules={hiddenTop:0,bonusLock:false,small2:false,small3:false,large5:false};
...
const canPour=(from,to)=>from!==to&&!isLocked(from)&&!isLocked(to)&&state[from]?.length&&state[to]&&state[to].length<capacities[to]&&(!state[to].length||state[from].at(-1)===state[to].at(-1));
const pour=(from,to)=>{if(!canPour(from,to))return false;const amount=Math.min(topRun(state[from]),capacities[to]-state[to].length),color=state[from].at(-1);for(let i=0;i<amount;i++){state[from].pop();state[to].push(color);}return true;};
const isLocked=index=>rules.bonusLock&&index===lock.index&&!lock.unlocked;
const snapshot=()=>({state:clone(state),unlocked:lock.unlocked});
const restore=value=>{state=clone(value.state);lock.unlocked=value.unlocked;};

function levelRules(value){
  if(value<=6)return{hiddenTop:0,bonusLock:false,small2:false,small3:false,large5:false};
  if(value<=12)return{hiddenTop:2,bonusLock:false,small2:false,small3:false,large5:false};
  if(value<=18)return{hiddenTop:0,bonusLock:true,small2:false,small3:false,large5:false};
  if(value<=24)return{hiddenTop:1,bonusLock:false,small2:true,small3:false,large5:false};
  if(value<=30)return{hiddenTop:1,bonusLock:false,small2:false,small3:true,large5:false};
  if(value<=36)return{hiddenTop:1,bonusLock:false,small2:false,small3:false,large5:true};
  return{hiddenTop:1,bonusLock:true,small2:true,small3:true,large5:true};
}
...
function addBonus(capacity){state.push([]);capacities.push(capacity);}
function setupPuzzle(puzzle,nextRules,value=level){
  state=clone(puzzle.state);capacities=state.map(()=>4);solution=(puzzle.solution||[]).map(m=>({...m}));rules={...nextRules};
...
function save(){localStorage.setItem('sand-logic-v27',JSON.stringify({state,initial,history,solution,capacities,level,moves,par,rules,lock}));}
function load(){try{const v=JSON.parse(localStorage.getItem('sand-logic-v27'));if(!v?.state?.length)return false;({state,initial,history,solution,capacities,level,moves,par,rules,lock}=v);selected=null;return true;}catch{return false;}}
function updateUi(){
...
function tap(index){
  if(busy||isLocked(index))return;if(selected===null){if(state[index].length){selected=index;render();}return;}if(selected===index){selected=null;render();return;}if(!canPour(selected,index)){selected=state[index].length?index:null;render();return;}
...
function showWin(){const rating=stars(moves,par),progress=JSON.parse(localStorage.getItem('sand-progress-v27')||'{}');progress[level]=Math.max(progress[level]||0,rating);localStorage.setItem('sand-progress-v27',JSON.stringify(progress));$('#winStars').textContent='★'.repeat(rating)+'☆'.repeat(3-rating);$('#winStats').textContent=`${moves} Züge · Par ${par}`;$('#winDialog').showModal();}

$('#undo').onclick=()=>{if(!history.length||busy)return;restore(history.pop());moves=Math.max(0,moves-1);selected=null;render();};
$('#restart').onclick=()=>{if(busy)return;state=clone(initial);history=[];selected=null;moves=0;lock.unlocked=!rules.bonusLock||complete(state[lock.trigger],lock.trigger);render();};
...
$('#levels').onclick=()=>{const grid=$('#levelGrid'),progress=JSON.parse(localStorage.getItem('sand-progress-v27')||'{}');grid.innerHTML='';for(let value=1;value<=60;value++){const b=document.createElement('button');b.textContent=progress[value]?`${value}\n${'★'.repeat(progress[value])}`:value;b.className=progress[value]?'done':'';b.onclick=()=>{startLevel(value);$('#levelDialog').close();};grid.appendChild(b);}$('#levelDialog').showModal();};
